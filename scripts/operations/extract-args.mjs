...
const TARGETS = [
  ...
  { key: "pauseController", file: "ArkBridgePauseController.sol", name: "ArkBridgePauseController" },
...
for (const target of TARGETS) {
  ...
  if (address === undefined) { console.log(`${target.key}: not in artifact`); continue; }

  let creation;
  try {
    creation = JSON.parse(readFileSync(`${OUT}/${target.file}/${target.name}.json`, "utf8"))
      .bytecode.object.replace(/^0x/, "");
  } catch { console.log(`${target.key}: no local build artifact`); continue; }
  ...
  if (txHash === undefined) { console.log(`${target.key}: creation tx unknown`); continue; }

  const tx = await client.getTransaction({ hash: txHash }).catch(() => null);
  if (tx === null) { console.log(`${target.key}: creation tx not retrievable`); continue; }
