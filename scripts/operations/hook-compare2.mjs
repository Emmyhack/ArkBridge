import { createPublicClient, http, toFunctionSelector, decodeAbiParameters } from "viem";
...
for (const [label, client, addr] of [
  ["ark    ", ark, "0x3155E3658841a0B9Ea59F3Cb3A55040481ad1cF7"],
  ["sepolia", sep, "0x7E3A670E7DafC4E92D7572b78d52A1E2F0A47553"],
]) {
  const code = (await client.getCode({ address: addr })).replace(/^0x/, "");
  const call = async (data) => {
    const r = await client.call({ to: addr, data }).catch((e) => null);
    return r?.data ?? null;
  };
  ...
  console.log(`  bytes ${code.length / 2}  codeEqualsSource(immutables blanked)=${normalise(code) === normalise(local)}`);
